...
messageForm.addEventListener('submit', function(event) {    //messageForm의 변수가 참조하는 "leave message" form에서 'submit' 버튼이 클릭되었을 때
    ...
    messageList.appendChild(newMessage) + newMessage.appendChild(removeButton) + newMessage.appendChild(editBu);//messageList변수에 newMessage라는 변수의 값을 '입양'시킨다. 여기서는 ul섹터 안에 들어가게 되는 것. 
